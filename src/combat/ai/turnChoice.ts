...
import type { CombatEntity, CombatMove, TurnChoice } from '../models/index.ts';
...
function predictiveTurnQueue(
	...
	const rotated = [
		...
		...roundOrder.slice(0, currentIndex + 1),
	];
	if (rotated.length === 0) {
		return [];
	}
...
function selectDeepSearchProjections(
	...
	for (
		const projection of ranked
			.filter(({ choice }) => isStatefulUtilityChoice(choice))
			.slice(0, reservedUtility)
	) {
...
function combatStateSignature(
	...
	const partyOrder = combat.entities.party.map(
		(entity) => entity.id).join(',');
	const encounterOrder = combat.entities.encounters.map(
		(entity) => entity.id).join(',');
...
function predictBestBranch(
	...
	if (projections.length === 0) {
		...
		if (next.actor) {
			const skipped = predictBestBranch(
				combat,
				awarenessTeam,
				next.actor,
				next.remaining,
	...
	for (const projection of projections) {
		const tacticalScore = stateInteractionHeuristic(
			combat,
			actor,
			awarenessTeam,
			projection.immediateChanges,
