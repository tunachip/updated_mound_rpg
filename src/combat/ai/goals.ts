...
import type { Status, TargetType } from '../../shared/index.ts';
...
function isBeneficialStatus(
	status: Status,
): status is typeof BENEFICIAL_STATUSES[number] {
	return BENEFICIAL_STATUSES.includes(status as typeof BENEFICIAL_STATUSES[number]);
}

function isHarmfulStatus(
	status: Status,
): status is typeof HARMFUL_STATUSES[number] {
	return HARMFUL_STATUSES.includes(status as typeof HARMFUL_STATUSES[number]);
}
...
function scanOperationTree(
	...
	switch (operation.name) {
		...
		case 'extendStatusTurns':
			if (
				status &&
				isBeneficialStatus(status) &&
				targetAllowsSupport(targetType)
			) {
				capabilities.beneficialStatuses.add(status);
			}
			if (
				status &&
				isHarmfulStatus(status) &&
				targetAllowsOffense(targetType)
			) {
